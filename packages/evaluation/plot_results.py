...
def main():
    ...
    parser.add_argument(
        "--results-file",
        help="Specific evaluation results file to plot"
    )
    parser.add_argument(
        "--output-dir",
    ...
    parser.add_argument(
        "--compare",
        nargs="+",
        help="Compare multiple result files (provide filenames)"
    )
    parser.add_argument(
        "--show",
        action="store_true",
    ...
    try:
        ...
        if args.compare:
            ...
            print(f"  Answer Relevance: {generation_metrics.get('answer_relevance', 0):.3f}")
            print(f"  Hallucination Rate: {generation_metrics.get('hallucination_rate', 0):.3f}")
