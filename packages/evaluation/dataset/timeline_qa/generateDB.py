...
def generateDb(
    ...
    templatefilename = templatefilename or os.path.join(data_folder, "templates.json")

    settings.init()

    settings.seed = seed
    settings.final_year = final_year
    settings.current_age = current_age
    ...
    with open(os.path.join(data_folder, "cities.csv"), "r") as f:
        obj = csv.reader(f, delimiter=",")
        for row in obj:
            settings.city_country_list.append(row)

    settings.college_list = []
    with open(os.path.join(data_folder, "colleges.csv"), "r") as f:
        obj = csv.reader(f, delimiter=",")
        for row in obj:
            settings.college_list.append(row)

    settings.major_list = []
    with open(os.path.join(data_folder, "majors.csv"), "r") as f:
        obj = csv.reader(f)
        for row in obj:
...
def main(argv):
    ...
    persona, episodicDB = generateDb(templatefilename, seed, final_year, current_age, verbose, category, directory)
