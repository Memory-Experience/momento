...
# Disable warning for E501 line length flake8
# flake8: noqa: E501

# Disable pyright warning
# type: ignore
...
class BatchedInferencePipeline:
    ...
    def transcribe(
        ...
        temperature: float | list[float] | tuple[float, ...] = None,
        ...
        if not clip_timestamps:
            if vad_filter:
                if vad_parameters is None:
                    ...
                    if "max_speech_duration_s" in vad_parameters:
        ...
        if language is None:
            ...
            if not self.model.model.is_multilingual and language != "en":
                self.model.logger.warning(
                    f"The current model is English-only but the language parameter is set to '{language}'; "
                    "using 'en' instead."
                )
                language = "en"
    ...
    def transcribe(
        ...
        if vad_filter and clip_timestamps == "0":
            ...
            if self.logger.isEnabledFor(logging.DEBUG):
                ...
                self.logger.debug(
                    f"VAD filter kept the following audio segments: {segments_str}"
                )
